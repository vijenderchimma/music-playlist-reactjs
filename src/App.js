...
class App extends Component {
  ...
  deleteSong = id => {
    ...
    const filteredData = songsList.filter(eachTrack => eachTrack.id !== id)
    this.setState({songsList: filteredData})
  }

  render() {
    const {searchInput, songsList} = this.state
    const filteredData = songsList.filter(eachSong =>
      eachSong.name.toLowerCase().includes(searchInput),
    )
    return (
      <div>
